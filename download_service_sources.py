...
def main() -> int:
    ...
    print(f"OSM {OSM['extract_date']}; Code-Point Open {CODEPOINT['release']}; PHS GP {PHS_GP['release']}; PHS dental {PHS_DENTAL['release']}; ERIC {ERIC['release']}")
    if args.dry_run: return 0
    sources: dict[str,dict] = {}
    for source in [download_osm(args.verify), download_codepoint(), download_small(ODS_GP, refresh=True), download_small(ODS_DENTAL, refresh=True), download_small(PHS_GP), download_small(PHS_DENTAL), download_small(ERIC)]:
